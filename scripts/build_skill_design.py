...
async function loadOnline() {
  ...
  try {
    ...
    if (Array.isArray(d.skills) && d.skills.length) {
      ...
      if (d.submit && d.submit.bin && d.submit.key) {
        submitCfg.bin = d.submit.bin;
        submitCfg.key = d.submit.key;
      }
    }
...
function submitVersion(i) {
  ...
  if (!submitCfg.bin) {
    openModal('提交设计: ' + (c.zh || c.en), `
      <div class="vcard">
        <div style="font-size:12px;color:#c62828;line-height:1.8">提交服务尚未配置，暂时无法在线提交。<br>
        替代方式：点「导出 JSON」把当前表格存到本地，发给维护者即可（导入时仍可作为版本记录）。</div>
      </div>`);
    return;
  }
  const savedAuthor = localStorage.getItem('sub_author') || '';
  openModal('提交设计: ' + (c.zh || c.en), `
    <div class="vcard">
      <div class="vrow"><span class="vkey">技能</span><span class="vval">${esc(c.zh || c.en)} (${esc(c.code)})</span></div>
      <div class="vrow"><span class="vkey">作者</span><input id="subAuthor" value="${esc(savedAuthor)}" placeholder="你的名字 / 昵称" style="flex:1;min-width:0"></div>
      <div class="vrow"><span class="vkey">备注</span><input id="subNote" placeholder="可选：设计思路、说明" style="flex:1;min-width:0"></div>
      <div style="font-size:11px;color:#888;margin:6px 0 10px">提交后你的设计会进入待合并列表，维护者确认后合入正式数据，所有人可见。</div>
...
async function doSubmit(c) {
  ...
  try {
    let items = [];
    const gr = await fetch('https://api.jsonbin.io/v3/b/' + submitCfg.bin + '/latest', {
      headers: { 'X-Master-Key': submitCfg.key }
    });
    if (gr.ok) {
      const gd = await gr.json();
      if (gd && gd.record && Array.isArray(gd.record.items)) items = gd.record.items;
    }
    items.push(payload);
    const pr = await fetch('https://api.jsonbin.io/v3/b/' + submitCfg.bin, {
      method: 'PUT',
      headers: { 'X-Master-Key': submitCfg.key, 'Content-Type': 'application/json' },
      body: JSON.stringify({ items: items })
    });
    if (pr.ok) {
      res.textContent = '提交成功！已进入待合并列表';
      res.style.color = '#2e7d32';
      loadSubmissions();
      setTimeout(closeModal, 1500);
    } else {
      res.textContent = '提交失败: HTTP ' + pr.status;
...
async function loadSubmissions() {
  const btn = document.getElementById('subListBtn');
  if (!btn) return;
  if (!submitCfg.bin) { btn.style.display = 'none'; return; }
  btn.style.display = '';
  try {
    const r = await fetch('https://api.jsonbin.io/v3/b/' + submitCfg.bin + '/latest', {
      headers: { 'X-Master-Key': submitCfg.key }
    });
    if (!r.ok) throw new Error(r.status);
    const d = await r.json();
    const items = (d.record && d.record.items) || [];
    btn.textContent = '待合并 ' + items.length;
...
function showSubmissions() {
  ...
  if (!items.length) { openModal('待合并提交', '<div style="padding:12px;color:#888">当前没有待合并的提交。</div>'); return; }
  const list = items.map((it) => {
    const d = it.design || {};
    return `<div class="vcard">
      <div class="vhead"><b>${esc(it.zh || it.en)}</b> (${esc(it.code)}) · ${esc(it.author)} · ${esc((it.date || '').slice(0, 10))}</div>
      ${it.note ? `<div class="vrow"><span class="vkey">备注</span><span class="vval">${esc(it.note)}</span></div>` : ''}
      <div class="vrow"><span class="vkey">费用</span><span class="vval">${esc(d.cost || '')}</span></div>
      <div class="vrow"><span class="vkey">类型</span><span class="vval">${esc(d.type || '')}</span></div>
      <div class="vrow"><span class="vkey">稀有度</span><span class="vval">${esc(d.rarity || '')}</span></div>
      <div class="vrow"><span class="vkey">升级前</span><span class="vval">${esc(d.effPre || '')}</span></div>
      <div class="vrow"><span class="vkey">升级后</span><span class="vval">${esc(d.effUpg || '')}</span></div>
    </div>`;
  }).join('');
  openModal('待合并提交 (' + items.length + ')', list + '<div style="font-size:11px;color:#888">这些提交等维护者确认后合入 designs.json，所有人可见。</div>');
}
...
new_header = '''<header>
  ...
  <button class="alt" id="subListBtn" style="display:none" onclick="showSubmissions()">待合并</button>
...
<div id="help" style="display:none;background:#fff;border-bottom:1px solid #d5d8dc;padding:10px 16px;font-size:12px;color:#555;line-height:1.8">
  ...
  2. 编辑某行（费用/类型/稀有度/效果）→ 点行内<b>「提交此版」</b> → 网页内填名字直接提交（不需要任何账号），提交进入待合并列表。<br>
  3. 点顶部<b>「待合并 N」</b>可查看所有人的待合并提交；维护者确认后合入 designs.json，自动成为该技能的版本，其他人点「版本 N」都能看到。<br>
